...
import { Share2, Bookmark, BookmarkCheck, Copy, Check } from "lucide-react"
...
export function ShareButtons({ title, slug }: ShareButtonsProps) {
    ...
    useEffect(() => {
        ...
        if (saved) {
            const articles = JSON.parse(saved) as string[]
            setIsSaved(articles.includes(slug))
        }
    }, [slug])
    ...
    return (
        <div className="flex items-center gap-3">
            ...
            <Button variant="outline" size="sm" onClick={handleSave}>
                {isSaved ? (
                    <>
                        <BookmarkCheck className="w-4 h-4 mr-2 text-purple-500" />
                        Saved
                    </>
                ) : (
                    <>
